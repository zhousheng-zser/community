        ...
        <el-row :gutter="24">
          ...
          <el-col :span="12">
            <el-form-item label="服务分类">
              <el-input v-model="form.category" placeholder="如：家政/维修/绿植" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
async function load() {
  loading.value = true
  try {
    const res = await request.get('/service-provider/me')
    form.value = res.data || {}
...
async function save() {
  saving.value = true
  try {
    await request.patch('/service-provider/profile', {
    ...
    info.name = form.value.shop_name || info.name
    localStorage.setItem('sp_shop_info', JSON.stringify(info))
  } catch (e) { ElMessage.error(e.message) }
