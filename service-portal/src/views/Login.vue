<template>
  <div class="login-page">
    <el-card class="login-box" shadow="always">
      ...
      <p class="footer-tip">此入口仅供已在小程序完成入驻的服务商使用</p>
    </el-card>
  </div>
</template>
...
async function handleLogin() {
  ...
  loading.value = true
  try {
    const res = await request.post('/auth/service-portal/login', {
    ...
    if (data.token) {
      ...
      if (data.shop) localStorage.setItem('sp_shop_info', JSON.stringify(data.shop))
      if (data.user) localStorage.setItem('sp_user_info', JSON.stringify(data.user))
...
.login-page {
  ...
  align-items: center;
  background: linear-gradient(135deg, #1a2035 0%, #2d4a7a 50%, #1a2035 100%);
}
.login-box {
  width: 420px;
  border-radius: 16px;
...
.title { margin: 0 0 4px; font-size: 24px; color: #1a2035; letter-spacing: 2px; }
.sub { margin: 0; color: #4a90d9; font-size: 14px; font-weight: 500; }
