...
function mapServiceRow(s) {
  ...
  return { id, title, priceText, unit, duration, category, onShelf: !!onShelf, mainImage, hasImage: !!String(rawImg).trim(), descShort, descForSearch: descFull };
}

Page({
  data: { fullList: [], list: [], keyword: '', loading: false, emptyTip: '暂无服务项目数据', summaryText: '' },
  onShow() { this.load(); },
  ...
  applyList() {
    ...
    if (k) { base = base.filter((x) => String(x.title).toLowerCase().includes(k) || String(x.category).toLowerCase().includes(k) || String(x.descForSearch || '').toLowerCase().includes(k)); }
  ...
  async load() {
    ...
    try {
      let res;
      try { res = await util.get('service-provider/my-services', { page: 1, limit: 100 }); }
      catch (e1) { if (e1 && (Number(e1.errno) === 404 || Number(e1.errno) === 501)) { res = await util.get('merchant/services', { page: 1, limit: 100 }); } else { throw e1; } }
      ...
      this.setData({ fullList, loading: false, emptyTip: '暂无服务项目，请先添加', summaryText });
      this.applyList();
    } catch (e) {
      this.setData({ loading: false });
      const errno = e && Number(e.errno);
      let emptyTip = '暂无服务项目数据';
      if (errno === 404 || errno === 501) { emptyTip = '服务项目接口待后端上线'; }
  ...
  onShelfChange(e) {
    ...
    wx.showModal({ title: next ? '上架' : '下架', content: next ? '确认上架该服务项目？用户端将可预约' : '确认下架？用户端将不可预约', success: (r) => { if (r.confirm) { this.submitShelf(id, next); } else { this.load(); } } });
  },
  async submitShelf(id, published) {
    try { await util.post(`service-provider/services/${id}/shelf`, { published, is_published: published ? 1 : 0 }); wx.showToast({ title: published ? '已上架' : '已下架', icon: 'success' }); await this.load(); }
    catch (err) { try { await util.post(`merchant/services/${id}/shelf`, { published }); wx.showToast({ title: published ? '已上架' : '已下架', icon: 'success' }); await this.load(); } catch (e2) { wx.showToast({ title: (e2 && e2.errmsg) || '接口待上线', icon: 'none' }); } }
  },
  ...
  goEdit(e) { const id = e.currentTarget.dataset.id; if (id == null) return; wx.showToast({ title: '编辑功能开发中', icon: 'none' }); },
  goAdd() { wx.showToast({ title: '添加功能开发中', icon: 'none' }); }
});
