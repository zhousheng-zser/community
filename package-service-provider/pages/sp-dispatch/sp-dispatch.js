...
Page({
  ...
  async loadPendingOrders() {
    ...
    try {
      const res = await util.get('service-provider/orders', { status: 'pending_accept', page: 1, limit: 50 });
  ...
  async confirmDispatch() {
    ...
    try {
      await util.post(`service-provider/orders/${selectedOrderId}/dispatch`, {
      ...
      success: async (res) => {
        if (res.confirm) {
          try {
            ...
            await util.post(`service-provider/orders/${orderId}/accept`, {});
            wx.hideLoading();
