...
**现产品与技术决策：家集市正式链路必须走微信真实收银台**，用户完成付款后由微信支付回调驱动订单状态，不再依赖「仅创建支付单 + 前端空轮询等待模拟成功」的方式。
...
> 更细的字段说明见：`doc/家集市_真实微信支付接入说明.md`  
...
| E2 | 文档 | 更新 `API_DOC.md` 家集市支付相关章节（如 12.4）；与 **第 4 节** 响应示例一致。 |
...
前端侧说明：`doc/家集市_真实微信支付接入说明.md`。

---

*文档维护：前端发起 · 第 6 次 · 主题：家集市切换真实微信支付（可直接转发后端）*
