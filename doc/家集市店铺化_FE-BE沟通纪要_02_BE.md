# 家集市店铺化 FE-BE 沟通纪要（后端视角·第 2 次）  
（给前端同事看的说明书，方便联调与后续扩展）

## 一、这次我们后端到底在做什么

先把话说明白：这次后端做的不是“帮前端凑几个接口”，而是把 **「家集市」当成一个独立的交易子系统** 来建设：
...
后续你在前端里，只要看到 `market` 域的接口，都可以认为是在走“家集市子系统”的读/写能力。
...
表结构完全按《家集市店铺化_后端需求(Node).md》里的 DDL 来做：
