...
Page({
  ...
  normalizeRecommendGoods(item, idx) {
    ...
    return {
      ...
      image,
      price,
      detailUrl: `/package-push/pages/push-product-detail/push-product-detail?id=${encodeURIComponent(String(id))}&shopId=${encodeURIComponent(String(this.data.shopId || ''))}&image=${encodeURIComponent(image)}&name=${encodeURIComponent(name)}&price=${encodeURIComponent(price)}`
    };
  },
  onLoad(options) {
