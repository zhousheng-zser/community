...
3. 若响应中明确标记模拟支付（如 `pay_mode` / `payment_mode` 为 `mock`/`virtual`，或 `virtual_pay === true`）：前端提示需开启正式支付。
4. 若**未解析到**完整 JSAPI 五参数：前端提示「未获取到微信支付参数，请检查后端支付配置」。
5. 参数齐全时调用 `wx.requestPayment`；用户支付成功/失败后，前端仍会轮询 `GET /api/v1/market/payments/status` 作为兜底展示。

**结论：后端必须保证 `payments/create` 在成功路径（`code === 0`）下，`data` 内可被解析出五参数，否则用户无法完成付款。**
