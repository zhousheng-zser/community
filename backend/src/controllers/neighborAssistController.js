...
exports.create = async (req, res) => {
  try {
    ...
    return ok(res, { id: row.id, order_id: row.id, status: row.status, assist_type: row.assist_type, assist_type_label: ASSIST_TYPE_LABELS[row.assist_type] || row.assist_type });
  } catch (e) {
...
exports.myList = async (req, res) => {
  try {
    const userId = req.user.id;
    const role = req.query.role || 'publisher'; // 默认查询发布的订单
    ...
    const where = {};
    if (role === 'publisher') {
      where.user_id = userId; // 我发布的订单
    } else if (role === 'helper') {
      where.assigned_worker_id = userId; // 我接的订单
    } else {
      // 默认查询所有相关订单
...
exports.detail = async (req, res) => {
  try {
    ...
    // 权限检查：只有发布人或接单人可以查看
    if (order.user_id !== userId && order.assigned_worker_id !== userId) {
      return fail(res, 403, '无权查看此订单');
    }
    ...
    // 判断当前用户角色
    const myRole = order.user_id === userId ? 'publisher' : 'helper';
    
    // 手机号处理：接单人确认后才可以查看对方手机号
    ...
    if (myRole === 'publisher' && worker) {
      ...
      peerInfo = {
        ...
        phone: isAccepted ? worker.phone : (worker.phone ? worker.phone.substring(0, 3) + '****' + worker.phone.substring(7) : ''),
        avatar_url: worker.avatar_url,
        isHelper: true
      };
    } else if (myRole === 'helper' && pub) {
      // 接单人看发布人
      peerInfo = {
        id: pub.id,
        nickname: pub.nickname,
        phone: pub.phone, // 发布人手机号对接单人是可见的
