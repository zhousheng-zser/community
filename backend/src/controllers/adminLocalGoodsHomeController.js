...
const LIST_DEFINITIONS = [
  ...
  { key: 'jiuzhou_haoshi', name: '九州好食', kind: 'channel', channel_key: 'jiuzhou_haoshi' },
  { key: 'jiuzhou_haowu', name: '九州好物', kind: 'channel', channel_key: 'jiuzhou_haowu' },
  { key: 'jiuzhou_haowei', name: '九州好味', kind: 'channel', channel_key: 'jiuzhou_haowei' },
...
async function resolveContext(def) {
  if (def.kind === 'channel' || def.kind === 'channel_tab') {
    let channel = null;
    let tabs = [];
    channel = await LgHomeChannel.findOne({ where: { channel_key: def.channel_key } });
    if (!channel) return { channel: null, tabs: [] };
    if (def.kind === 'channel_tab') {
      tabs = await LgHomeChannelTab.findAll({
