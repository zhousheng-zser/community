  ...
  <div class="sp-container">
    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      ...
      <el-tab-pane label="服务项目" name="services">
        <div class="toolbar">
          <el-select v-model="selectedProviderId" placeholder="选择服务商（默认全部）" clearable filterable
            style="width:220px" @change="fetchServices">
            <el-option label="全部服务商" :value="null" />
...
async function fetchAllProviders() {
  try {
    ...
    allProviders.value = (res.data || []).map(p => ({ id: p.id, shop_name: p.shop_name }))
  } catch {}
}
...
async function fetchServices() {
  ...
  try {
    const res = selectedProviderId.value
      ? await request.get(`/admin/service-providers/${selectedProviderId.value}/services`)
      : await request.get('/admin/sp-services')
